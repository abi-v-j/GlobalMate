...
import {
  ...
  Chip, InputAdornment, CircularProgress, Snackbar, Alert,
} from "@mui/material";
...
const glassPanel = {
  background: "linear-gradient(160deg, rgba(13,61,95,0.95) 0%, rgba(8,39,63,0.97) 100%)",
  border: "1px solid rgba(255,255,255,0.10)",
  borderRadius: "24px",
  boxShadow: "0 24px 64px rgba(10,50,80,0.30), inset 0 1px 0 rgba(255,255,255,0.10)",
...
const ExpenseCategory = () => {
  ...
  const fetchData = async () => {
    ...
    if (!error) setExpCategoryData(data || []);
    setLoading(false);
  };
  ...
  const handleSave = async () => {
    if (editId) {
      ...
      const { error } = await supabase
        .from("tbl_expenseCategory")
        .update({ Category_name: editName.trim() })
        .eq("id", editId);
      if (!error) { notify("Category updated!"); cancelEdit(); await fetchData(); }
      ...
      const { error } = await supabase
        .from("tbl_expenseCategory")
        .insert([{ Category_name: value.trim() }]);
      if (!error) { notify("Category saved!"); setValue(""); await fetchData(); }
  ...
  /* ── EDIT helpers */
  const startEdit = (row) => { setEditId(row.id); setEditName(row.Category_name || ""); };
  const cancelEdit = () => { setEditId(null); setEditName(""); };

  /* ── DELETE */
  const handleDelete = async (id) => {
    setDeletingId(id);
    const { error } = await supabase.from("tbl_expenseCategory").delete().eq("id", id);
    if (!error) { notify("Deleted.", "info"); await fetchData(); }
    else notify("Delete failed.", "error");
    setDeletingId(null);
  };

  const filtered = expCategoryData.filter((d) =>
    d.Category_name?.toLowerCase().includes(search.toLowerCase())
  );

  // shared MUI TextField sx factory
  const fieldSx = {
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <div style={{
            ...
            <AttachMoneyRoundedIcon sx={{ color: "white", fontSize: 22 }} />
          </div>
          <div>
            <Typography sx={{
              fontFamily: "'Playfair Display', serif", fontSize: 22,
              fontWeight: 700, color: "#0A3250", lineHeight: 1.2,
            }}>
              Expense Category
            </Typography>
            <Typography sx={{ fontSize: 12.5, color: "rgba(10,50,80,0.48)", fontFamily: "'DM Sans', sans-serif" }}>
              Create, edit and manage expense types
            </Typography>
          </div>
        </div>
        <Chip
          label={`${expCategoryData.length} Total`}
          sx={{
            background: "linear-gradient(135deg, #0d3d5f, #1a6090)",
      ...
      <div style={{ display: "flex", gap: 24, alignItems: "flex-start", flexWrap: "wrap" }}>
        ...
        >
          ...
          <div style={{ position: "relative", zIndex: 1, padding: "26px 22px" }}>
            ...
            {/* form header — animates between Add / Edit */}
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 22 }}>
              <AnimatePresence mode="wait">
                <motion.div
                  ...
                  style={{
                    ...
                    border: `1px solid ${editId ? "rgba(251,191,36,0.28)" : "rgba(255,255,255,0.18)"}`,
                    display: "flex", alignItems: "center", justifyContent: "center",
                  }}
                >
                  {editId
              ...
              <AnimatePresence mode="wait">
                ...
                >
                  <Typography sx={{ color: "white", fontFamily: "'DM Sans', sans-serif", fontWeight: 700, fontSize: 15, lineHeight: 1.2 }}>
                    {editId ? "Edit Category" : "New Category"}
                  </Typography>
                  <Typography sx={{ color: "rgba(255,255,255,0.38)", fontFamily: "'DM Sans', sans-serif", fontSize: 11 }}>
                    {editId ? "Modify existing entry" : "Add a new expense type"}
                  </Typography>
                </motion.div>
              </AnimatePresence>
            </div>
            ...
            <Typography sx={{
              ...
              fontSize: 10.5, letterSpacing: "0.1em", textTransform: "uppercase", mb: 1,
            }}>
              Category Name
            </Typography>
            ...
            <TextField
              ...
              variant="outlined"
              placeholder={editId ? "Edit category name…" : "e.g. Travel, Utilities…"}
              ...
              InputProps={{
                startAdornment: (
                  <InputAdornment position="start">
                    <CategoryRoundedIcon sx={{ color: "rgba(255,255,255,0.32)", fontSize: 18 }} />
                  </InputAdornment>
                ),
              }}
              sx={{ mb: 2.5, ...fieldSx }}
            ...
            <div style={{ display: "flex", gap: 8 }}>
              ...
              <motion.button
                ...
                style={{
                  ...
                  border: `1px solid ${editId ? "rgba(251,191,36,0.28)" : "rgba(255,255,255,0.15)"}`,
                  background: editId
                  ...
                  opacity: (editId ? !editName.trim() : !value.trim()) || saving ? 0.45 : 1,
              ...
              <AnimatePresence>
                {editId && (
                  <motion.button
                    ...
                    onClick={cancelEdit}
                    style={{
                      height: 44, borderRadius: 13, overflow: "hidden", cursor: "pointer",
        ...
        >
          <AmbientBlob top={-40} right={-30} size={240} opacity={0.10} />

          <div style={{ position: "relative", zIndex: 1, padding: "26px 22px" }}>

            {/* table toolbar */}
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
              <Typography sx={{ color: "white", fontFamily: "'DM Sans', sans-serif", fontWeight: 700, fontSize: 15 }}>
                All Categories
                <Typography component="span" sx={{ ml: 1.2, fontSize: 12, color: "rgba(255,255,255,0.32)", fontWeight: 400 }}>
            ...
            <div style={{
              ...
              padding: "7px 12px", marginBottom: 6, borderRadius: 10,
              background: "rgba(255,255,255,0.05)",
            }}>
              {["#", "Category Name", "Actions"].map((h, idx) => (
                <Typography key={h} sx={{
                  color: "rgba(255,255,255,0.32)", fontFamily: "'DM Sans', sans-serif",
            ...
            <div
              ...
              style={{ display: "flex", flexDirection: "column", gap: 5, maxHeight: 400, overflowY: "auto" }}
            >
              {loading ? (
                ...
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                  ...
                  <Typography sx={{ color: "rgba(255,255,255,0.2)", fontFamily: "'DM Sans', sans-serif", fontSize: 13 }}>
                    {search ? "No results found" : "No categories yet — add one!"}
                  </Typography>
                </motion.div>
              ) : (
                  ...
                  {filtered.map((row, i) => {
                    ...
                    return (
                      <motion.div
                        ...
                        style={{
                          display: "grid", gridTemplateColumns: "44px 1fr 80px",
                          alignItems: "center", padding: "10px 12px", borderRadius: 14,
                          background: isEditing ? "rgba(251,191,36,0.08)" : "rgba(255,255,255,0.04)",
                          border: `1px solid ${isEditing ? "rgba(251,191,36,0.20)" : "rgba(255,255,255,0.07)"}`,
                      ...
                      >
                        ...
                        <Typography sx={{
                          ...
                          fontFamily: "'DM Sans', sans-serif",
                          fontSize: 13.5, fontWeight: isEditing ? 600 : 500,
